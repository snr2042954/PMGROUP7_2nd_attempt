...
def compute_metrics(discovered: set, gold: set, activities: set = None):
    """Compute precision, recall, F1-score, and confusion matrix between discovered and gold relations.
    
    Args:
        discovered: Set of discovered relations (tuples)
        gold: Set of gold standard relations (tuples)
        activities: Set of all activities (for calculating TN). If None, TN will be None.
    
    Returns:
        tuple: (precision, recall, f1, tp, fp, fn, tn)
    """
    intersection = discovered & gold
    tp = len(intersection)  # True Positives: in both discovered and gold
    fp = len(discovered - gold)  # False Positives: in discovered but not in gold
    fn = len(gold - discovered)  # False Negatives: in gold but not in discovered
    
    # True Negatives: all possible relations not in discovered and not in gold
    tn = None
    if activities is not None:
        # Universe: all possible pairs (a, b) where a != b
        all_possible = {(a, b) for a in activities for b in activities if a != b}
        tn = len(all_possible - discovered - gold)
...
def evaluate_pm4py_alpha(dataset: str, log_path: str):
    ...
    precision, recall, f1, tp, fp, fn, tn = compute_metrics(relations, gold_relations, gold.activities)
    return {
        "precision": precision,
        "recall": recall,
        "f1": f1,
...
def evaluate_custom_alpha(dataset: str, log_path: str, abs_threshold: int = 1, rel_threshold: float = 0.0):
    ...
    precision, recall, f1, tp, fp, fn, tn = compute_metrics(custom_relations, gold_relations, gold.activities)
